...
class VoxeetSDK {
  ...
  initializeWithRefresh (accessToken, refreshToken) {
    if(!this.refreshAccessTokenCallback) {
      console.warn("initializing for the first time");
      this.refreshAccessTokenCallback = () => {
        console.warn("retrieving the accessToken from the refreshToken...", refreshToken);
        refreshToken()
      ...
      DeviceEventEmitter.addListener("refreshToken", (e: Event) => {
        this.refreshAccessTokenCallback();
      });
    }

    return RNVoxeetConferencekit.initializeWithRefresh(accessToken);
  }
